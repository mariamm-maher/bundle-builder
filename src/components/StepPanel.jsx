...
export default function StepPanel({ step, isOpen, isHighlighted, onToggle, onNext, catalog, activeVariants, setActiveVariants, onVariantChange, quantities, setQty, getActiveKey, icons }) {
  ...
  return (
    <article
      className={cx(
        'relative overflow-hidden border-t border-[#e0e4eb] bg-white',
        'max-[640px]:m-0 max-[640px]:overflow-visible max-[640px]:border-0 max-[640px]:border-t max-[640px]:border-[#d1d5db] max-[640px]:rounded-none max-[640px]:bg-white',
        isOpen && 'rounded-[8px] border-0 bg-[#EDF4FF] px-[14px] pb-[18px]',
      )}
    >
      <div className="hidden h-[26px] items-center px-[18px] text-[13px] font-extrabold leading-none tracking-[2px] text-[#6f737d] max-[640px]:flex">
        STEP {step.order} OF 4
      </div>
      <div className="hidden max-[640px]:block h-[1px] bg-[#e0e4eb]"></div>
      <StepHeader step={step} isOpen={isOpen} selectedCount={isOpen ? undefined : 0} onToggle={onToggle} icons={icons} />
